...
public class CalendarEventDetailListAdapter extends ArrayAdapter {
    ...
    public CalendarEventDetailListAdapter(@NonNull Context context, @LayoutRes int resource, int clickedPosition, int clickedSubPosition) {
    ...
    public int getCount() {
        return ((ArrayList<CalendarDataModel>) (CalendarAllEventsListViewAdapter.displayCalendarData.values().toArray()[clickedPosition])).get(clickedSubPosition).allData.size();
    }

    @Nullable
    @Override
    public Object getItem(int position) {
        return ((ArrayList<CalendarDataModel>) (CalendarAllEventsListViewAdapter.displayCalendarData.values().toArray()[clickedPosition])).get(clickedSubPosition).allData.get(position);
    }

    public View getView(int position, View convertView, ViewGroup parent) {
        View rowView = convertView;
        DetailEventListAdapter detailEventListAdapter;
        if (rowView == null) {
            LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            rowView = inflater.inflate(R.layout.detail_event_list_item, null);
            detailEventListAdapter = new DetailEventListAdapter();
            ...
            detailEventListAdapter = (DetailEventListAdapter) rowView.getTag();
        String itemDetail = (String) getItem(position);
        if (itemDetail == null)
            itemDetail = "N/A";
        detailEventListAdapter.eventItemName.setText(dataDetail[position]);
        detailEventListAdapter.eventItemDetail.setText(itemDetail.replace("&nbsp;", " ").replace("&amp;", "&"));
        return rowView;
    }
